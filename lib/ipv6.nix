let
  ...
  mk = ws: { _type = "ipv6"; words = ws; };

  # ===== Conversion =====

  fromWords = ws:
    if !(builtins.isList ws) || builtins.length ws != 4
    then builtins.throw "libnet.ipv6.fromWords: expected list of 4 u32 ints"
    else
      let
        invalid = builtins.any
          (w: !(builtins.isInt w) || w < 0 || w > bits.mask32) ws;
      in
        if invalid
        then builtins.throw "libnet.ipv6.fromWords: each word must be int in [0, 2^32 - 1]"
        else mk ws;

  toWords = ip: ip.words;

  fromGroups = gs:
    if !(builtins.isList gs) || builtins.length gs != 8
    then builtins.throw "libnet.ipv6.fromGroups: expected list of 8 u16 ints"
    else
      let
        invalid = builtins.any
          (g: !(builtins.isInt g) || g < 0 || g > bits.mask16) gs;
      in
        if invalid
        then builtins.throw "libnet.ipv6.fromGroups: each group must be int in [0, 65535]"
        else
          let
            g = i: builtins.elemAt gs i;
            pair = i: j: (g i) * bits.pow2_16 + (g j);
          in
            mk [ (pair 0 1) (pair 2 3) (pair 4 5) (pair 6 7) ];
  ...
  fromBytes = bs:
    if !(builtins.isList bs) || builtins.length bs != 16
    then builtins.throw "libnet.ipv6.fromBytes: expected list of 16 u8 ints"
    else
      let
        invalid = builtins.any
          (b: !(builtins.isInt b) || b < 0 || b > 255) bs;
      in
        if invalid
        then builtins.throw "libnet.ipv6.fromBytes: each byte must be int in [0, 255]"
        else
          let
            b = i: builtins.elemAt bs i;
            quad = i: (b i) * bits.pow2_24 + (b (i + 1)) * bits.pow2_16
                    + (b (i + 2)) * bits.pow2_8 + (b (i + 3));
          in
            mk [ (quad 0) (quad 4) (quad 8) (quad 12) ];
  ...
  parseHexGroups = strs:
    let results = map parse'.hexGroup strs;
    in if builtins.any (v: v == null) results then null else results;
  ...
  expandPartsToGroups = parts:
    let n = builtins.length parts; in
      if n == 0 then []
      else if !(v4OnlyLast parts) then null
      else
        let
            ...
            in
              if !v4Ok then null
                ...
                let
                  ...
                  prefixParts = if n == 1
                    then []
                    else builtins.genList (i: builtins.elemAt parts i) (n - 1);
                  prefixGroups = parseHexGroups prefixParts;
                in
                  if prefixGroups == null then null
                  else prefixGroups ++ [ g0 g1 ];

  tryParse = s:
    if !(builtins.isString s)
    then types.tryErr "libnet.ipv6.parse: input must be a string"
    else if s == ""
    then types.tryErr "libnet.ipv6.parse: empty string"
    else
      let
        dcCount = parse'.countOccurrences "::" s;
      in
        if dcCount > 1
        then types.tryErr "libnet.ipv6.parse: more than one \"::\" in \"${s}\""
        else
          let
            groups =
              if dcCount == 0 then
                ...
                in
                  if leftG == null || rightG == null then null
                  else
                    let total = builtins.length leftG + builtins.length rightG;
                    in
                      if total > 7 then null  # "::" must represent at least 1 zero group
                      else
                        let zeros = builtins.genList (_: 0) (8 - total);
                        in leftG ++ zeros ++ rightG;
          in
            if groups == null
            then types.tryErr "libnet.ipv6.parse: invalid \"${s}\""
            else types.tryOk (fromGroups groups);
  ...
  toString = ip:
    ...
    then
      ...
      in
        if run.len == 0
        then builtins.concatStringsSep ":" groupStrs
        else
          let
            prefix = builtins.genList (i: builtins.elemAt groupStrs i) run.start;
            suffix = builtins.genList
              (i: builtins.elemAt groupStrs (run.start + run.len + i))
              (8 - run.start - run.len);
            prefixStr = builtins.concatStringsSep ":" prefix;
            suffixStr = builtins.concatStringsSep ":" suffix;
          in "${prefixStr}::${suffixStr}";

  toStringCompressed = toString;

  toStringExpanded = ip:
    builtins.concatStringsSep ":" (map fmt.hex4 (toGroups ip));
  ...
  toArpa = ip:
    let
      bs = toBytes ip;
      nibblesOf = b: [ (fmt.hex1 (bits.shr 4 b)) (fmt.hex1 (builtins.bitAnd b 15)) ];
      nibbles = builtins.concatMap nibblesOf bs;
      # Reverse 32 nibbles
      reversed = builtins.genList
        (i: builtins.elemAt nibbles (31 - i))
        32;
    in
  ...
  isUnspecified = ip: ip.words == [ 0 0 0 0 ];
  isLoopback    = ip: ip.words == [ 0 0 0 1 ];
  ...
  isDocumentation = ip:
    w 0 ip == 536939960
    || bits.shr 12 (w 0 ip) == 262128;

  # ::ffff:0:0/96 — w0==0, w1==0, w2==0xffff (65535)
  isIpv4Mapped = ip:
    w 0 ip == 0 && w 1 ip == 0 && w 2 ip == 65535;

  # ::/96 — w0==0, w1==0, w2==0
  isIpv4Compatible = ip:
    w 0 ip == 0 && w 1 ip == 0 && w 2 ip == 0;

  # 2002::/16 — upper 16 bits of w0 = 0x2002 = 8194
  is6to4 = ip: bits.shr 16 (w 0 ip) == 8194;

  isBogon = ip:
    isUnspecified ip || isLoopback ip || isLinkLocal ip
    || isUniqueLocal ip || isMulticast ip || isDocumentation ip;

  # Spec: isGlobal means "none of the above" — wider set than isBogon,
  # additionally excluding isIpv4Mapped, isIpv4Compatible, is6to4.
  isGlobal = ip:
    !(isBogon ip || isIpv4Mapped ip || isIpv4Compatible ip || is6to4 ip);
  ...
  fromIpv4Mapped = v4:
    if !(types.isIpv4 v4)
    then builtins.throw "libnet.ipv6.fromIpv4Mapped: expected ipv4 value"
    else mk [ 0 0 65535 v4.value ];

  toIpv4Mapped = ip:
    if !(isIpv4Mapped ip)
    then builtins.throw "libnet.ipv6.toIpv4Mapped: address is not in ::ffff:0:0/96"
    else { _type = "ipv4"; value = w 3 ip; };
  ...
  fromEui64 = cidrVal: macVal:
    let
      isValidCidr = types.isCidr cidrVal
        && cidrVal.address._type == "ipv6";
    in
      if !isValidCidr
      then builtins.throw "libnet.ipv6.fromEui64: first argument must be an IPv6 cidr"
      else if !(types.isMac macVal)
      then builtins.throw "libnet.ipv6.fromEui64: second argument must be a mac"
      else if cidrVal.prefix > 64
      then builtins.throw "libnet.ipv6.fromEui64: CIDR prefix must be <= 64, got /${builtins.toString cidrVal.prefix}"
      else
        let
          ...
          applyMask = wv: keep:
            if keep <= 0 then 0
            else if keep >= 32 then wv
            else bits.shl (32 - keep) (bits.shr (32 - keep) wv);
  ...
  addU63 = n: ip:
    ...
    in
      if r0.carry == 1
      then builtins.throw "libnet.ipv6.add: overflow beyond 2^128"
      else mk [ r0.sum r1.sum r2.sum r3.sum ];
  ...
  subU63 = n: ip:
    ...
    in
      if r0.borrow == 1
      then builtins.throw "libnet.ipv6.sub: underflow below 0"
      else mk [ r0.diff r1.diff r2.diff r3.diff ];

  add = n: ip:
    if n == 0 then ip
    else if n > 0 then addU63 n ip
    else subU63 (0 - n) ip;
  ...
  subMultiWord = xa: xb:
    ...
    in {
      words = [ s0.diff s1.diff s2.diff s3.diff ];
  ...
  lower64OrThrow = ws:
    ...
    in
      if w0 != 0 || w1 != 0
      then builtins.throw "libnet.ipv6.diff: result exceeds signed 63-bit int range"
      else if w2 >= 2147483648  # 2^31
      then builtins.throw "libnet.ipv6.diff: result exceeds signed 63-bit int range"
      else w2 * bits.pow2_32 + w3;

  # diff b - a, expressed as Int. Throws if difference exceeds signed 63-bit int range.
  diff = a: b:
    let r = subMultiWord b.words a.words;
    in
      if r.finalBorrow == 0
      then lower64OrThrow r.words
      else
        let r2 = subMultiWord a.words b.words;
        in 0 - (lower64OrThrow r2.words);
  ...
  compare = a: b:
    let
      cmpWord = i:
        let aw = builtins.elemAt a.words i;
            bw = builtins.elemAt b.words i;
        in if aw < bw then -1 else if aw > bw then 1 else 0;
      go = i:
        if i == 4 then 0
        else let c = cmpWord i;
             in if c != 0 then c else go (i + 1);
    in go 0;
...
in
{
  inherit fromWords toWords fromGroups toGroups fromBytes toBytes;
  inherit parse tryParse toString toStringCompressed toStringExpanded toStringBracketed toArpa;
  inherit isValid is;
  inherit isUnspecified isLoopback isLinkLocal isUniqueLocal isMulticast;
  inherit isDocumentation isIpv4Mapped isIpv4Compatible is6to4 isGlobal isBogon;
