let
  ...
  hex2 = n:
    (hex1 (bits.shr 4 n)) + (hex1 (builtins.bitAnd n 15));
  ...
  hex = n:
    if n == 0 then "0"
      ...
      let
        go = v: acc:
          if v == 0 then acc
          else go (bits.shr 4 v) ((hex1 (builtins.bitAnd v 15)) + acc);
      in go n "";
  ...
  longestZeroRun = groups:
    let
      ...
      scan = i: bestStart: bestLen: curStart: curLen:
        if i == n then
          if bestLen >= 2
          then { start = bestStart; len = bestLen; }
          else { start = -1; len = 0; }
        else
          let g = builtins.elemAt groups i;
          in
            if g == 0 then
              let
                nCurLen = curLen + 1;
              ...
              in
                if nCurLen > bestLen
                then scan (i + 1) nCurStart nCurLen nCurStart nCurLen
                else scan (i + 1) bestStart bestLen nCurStart nCurLen
            else
...
in
{
  inherit hex1 hex2 hex4 hex longestZeroRun;
}
