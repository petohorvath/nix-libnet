let
  ...
  hasTag = tag: v:
    builtins.isAttrs v && v ? _type && v._type == tag;
  ...
  tryOk  = value: { success = true;  inherit value; error = null; };
  tryErr = error: { success = false; value = null;  inherit error; };

  ensureTag = tag: ctx: v:
    if hasTag tag v then v
    else builtins.throw "libnet: ${ctx}: expected ${tag} value, got ${
      if builtins.isAttrs v && v ? _type
      then "${v._type} value"
...
{
  inherit tags hasTag;
  inherit isIpv4 isIpv6 isMac isCidr isPort isPortRange isEndpoint isListener isRange isInterface isIp;
