let
  ...
  mk = v: { _type = "ipv4"; value = v; };

  # ===== Conversion =====

  fromInt = n:
    if !(builtins.isInt n) || n < 0 || n > bits.mask32
    then builtins.throw "libnet.ipv4.fromInt: value out of range [0, 4294967295]: ${builtins.toString n}"
    else mk n;

  toInt = ip: ip.value;

  fromOctets = os:
    if !(builtins.isList os) || builtins.length os != 4
    then builtins.throw "libnet.ipv4.fromOctets: expected list of 4 ints"
    else
      let
        ...
        invalid = builtins.any
          (v: !(builtins.isInt v) || v < 0 || v > 255) os;
      in
        if invalid
        then builtins.throw "libnet.ipv4.fromOctets: each octet must be int in [0, 255]"
        else mk (a * bits.pow2_24 + b * bits.pow2_16 + c * bits.pow2_8 + d);
  ...
  tryParse = s:
    ...
    then types.tryErr "libnet.ipv4.parse: input must be a string"
    else
      let
      ...
      in
        if len != 4
        then types.tryErr "libnet.ipv4.parse: must have 4 octets, got ${builtins.toString len}: \"${s}\""
        else
          let
            oct = i: parse'.octet (builtins.elemAt parts i);
            a = oct 0; b = oct 1; c = oct 2; d = oct 3;
          in
            if a == null || b == null || c == null || d == null
            then types.tryErr "libnet.ipv4.parse: invalid octet in \"${s}\""
            else types.tryOk (fromOctets [ a b c d ]);
  ...
  toString = ip:
    builtins.concatStringsSep "." (map builtins.toString (toOctets ip));
  ...
  isLoopback = ip:
    ip.value >= 127 * bits.pow2_24
    && ip.value <= 128 * bits.pow2_24 - 1;
  ...
  isMulticast = ip:
    ip.value >= 224 * bits.pow2_24
    && ip.value <= 240 * bits.pow2_24 - 1;
  ...
  isReserved = ip:
    ip.value >= 240 * bits.pow2_24
    && ip.value <= bits.mask32 - 1;
  ...
  isBogon = ip:
    isLoopback ip || isPrivate ip || isLinkLocal ip || isMulticast ip
    || isReserved ip || isDocumentation ip || isUnspecified ip || isBroadcast ip;
  ...
  add = n: ip:
    let r = ip.value + n;
    in
      if r < 0 || r > bits.mask32
      then builtins.throw "libnet.ipv4.add: result out of range [0, 4294967295]"
      else mk r;
...
in
{
  inherit fromInt toInt fromOctets toOctets;
  inherit parse tryParse toString toArpa;
  inherit isValid is;
  inherit isLoopback isPrivate isLinkLocal isMulticast isBroadcast isUnspecified isReserved isDocumentation isGlobal isBogon;
