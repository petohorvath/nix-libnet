let
  formatValue = v:
    ...
    in if r.success then r.value else builtins.toString v;
  ...
  runTests = tests:
    ...
    in
      if failures == []
      then { passed = builtins.length (builtins.attrNames tests); }
  ...
  prefix = p: ts:
    builtins.listToAttrs
      (map (n: { name = "${p}.${n}"; value = ts.${n}; })
           (builtins.attrNames ts));
in {
  inherit runTests throws succeeds prefix;
}
