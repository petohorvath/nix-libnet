...
let
  ...
  tryImport = name:
    let p = ./. + "/${name}";
    in if builtins.pathExists p then import p { inherit harness; } else {};
  ...
  typeTests =
    if lib != null && builtins.pathExists (./. + "/types.nix")
    then prefix "types" (import ./types.nix { inherit harness lib; })
    else {};
